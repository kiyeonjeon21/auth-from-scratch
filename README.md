...
인증·인가 방식들을 **라이브러리 없이 하나씩 만들어보고 나란히 비교하는** 학습 저장소.

목표는 프로덕션 코드가 아니다.
각 방식이 **앞의 무엇이 부족해서 나왔고, 무엇을 포기했는지** 를 손으로 확인하는 것이다.
...
**여기는 OIDC 바깥을 본다.** 세션, Passkey, 요청 서명, 로그아웃, SAML.
저쪽이 깊이라면 여기는 넓이다.
...
코드가 도는 것은 완료 조건이 아니다. 넷을 다 채워야 끝난다.

1. 동작하는 최소 코드
2. `ANSWERS.md` — 질문에 자기 말로 답변. 스펙 인용은 답이 아니다
3. 공격 재현 1개 — 검증을 한 줄 빼면 어떻게 뚫리는지 실제로 실행
4. **`notes/comparison.md` 에 한 줄 추가**
