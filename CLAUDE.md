...
A study repo for OIDC / OAuth 2.0 / SAML / Token Exchange.
Chapters implement the protocols by hand so the reader can see what a library normally hides.
...
```
00-first-login-trace/     library-based login + wire capture (the only chapter that uses a library)
01-jwt-by-hand/ ... 08/   hand-rolled implementations
internal/wiretrace/       shared HTTP recorder that renders annotated Markdown traces
docker/keycloak/          realm-as-code for the local IdP
notes/                    spec reading notes
```
...
A chapter is done when all three exist:
